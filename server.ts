...
function getKnowledgeResponse(message: string): string {
  ...
  if (
    lowerMsg.includes("study in europe") ||
    lowerMsg.includes("study in the uk") ||
    lowerMsg.includes("study in europe or the uk")
  ) {
  ...
  if (
    lowerMsg.includes("visit visa") ||
    lowerMsg.includes("tourist visa") ||
    lowerMsg.includes("ভ্রমণ")
  ) {
  ...
  if (
    lowerMsg.includes("documents are required for a student visa") ||
    lowerMsg.includes("student visa docs") ||
    lowerMsg.includes("student visa documents")
  ) {
...
• Police Clearance Certificate & Health/Medical Insurance.`;
  ...
  if (
    lowerMsg.includes("work permit") ||
    lowerMsg.includes("work visa") ||
    lowerMsg.includes("job visa")
  ) {
  ...
  if (
    lowerMsg.includes("indian medical") ||
    lowerMsg.includes("medical visa")
  ) {
...
Contact Euro Express at 01798483565 for quick processing.`;
  ...
  if (
    lowerMsg.includes("air ticket") ||
    lowerMsg.includes("air ticketing") ||
    lowerMsg.includes("flight") ||
    lowerMsg.includes("티켓")
  ) {
  ...
  if (
    lowerMsg.includes("required documents") ||
    lowerMsg.includes("what documents do i need")
  ) {
  ...
  try {
    ...
    if (message.length > 2000) {
      res
        ...
        .json({
          error:
            "Message length exceeds maximum allowable limit (2000 characters)",
        });
      return;
    }
    ...
    if (groq) {
      try {
        ...
        console.warn("Groq API call warning/fallback to Gemini:", groqError);
      }
    }

    // 2. Try Gemini AI fallback if Groq failed or not available
    ...
    if (gemini) {
      try {
        ...
        console.warn("Gemini API call warning/fallback to local knowledge:", geminiError);
      }
    }

    // 3. Fallback to rich local knowledge response
...
// API Endpoint to fetch Google Reviews securely
app.get("/api/google-reviews", async (req, res) => {
  try {
    const apiKey = process.env.GOOGLE_PLACES_API_KEY;
    if (!apiKey) {
      return res.status(400).json({ error: "Google Places API key is not configured in environment variables." });
    }

    // The verified Place ID extracted from the provided short link
    const placeId = "ChIJP7ij-8t8PqsRdyBWoqh6tk8"; 

    // Fetch using the Google Places API (New)
    ...
    // Default reviews list matching exactly what is seen in the real Google Business Profile
    const fallbackReviews = [
      { 
        id: '1', 
        name: 'Robiul Islam', 
        rating: 5, 
        comment: '"Highly recommend Euro Express Travels! Their service is incredibly professional, fast, and reliable. They provided...', 
        date: '9 mins ago' 
      },
      { 
        id: '2', 
        name: 'Fokhrul Islam', 
        rating: 5, 
        comment: 'One of the most trusted travel agencies in Sylhet division. Their visa guidance and file processing are 100% genuine and hassle-free. Got my Schengen visa support on time!', 
        date: '1 week ago' 
      },
      { 
        id: '3', 
        name: 'josim roni', 
        rating: 5, 
        comment: 'Best visa processing and travel agency in Habiganj. Very professional, fast service and highly trusted consultancy for Europe and tourist visas. Highly recommended!', 
        date: '1 week ago' 
      },
      { 
        id: '4', 
        name: 'Abcd Efg', 
        rating: 5, 
        comment: 'ভিসা প্রসেসিং নিয়ে অনেক কিছু জানলাম। তাদের পরামর্শ বেশ helpful ছিল।', 
        date: '3 weeks ago' 
      },
      { 
        id: '5', 
        name: 'Raju Ahmed', 
        rating: 5, 
        comment: 'Excellent customer service. They helped me with my ticket booking and visa processing very smoothly.', 
        date: '2 months ago' 
      },
      { 
        id: '6', 
        name: 'Sabbir Hossain', 
        rating: 5, 
        comment: 'Highly reliable and trustworthy. Their response time is very quick.', 
        date: '3 months ago' 
      },
      { 
        id: '7', 
        name: 'Kamrul Islam', 
        rating: 5, 
        comment: 'Very professional behavior and accurate information provided. Best in town.', 
        date: '4 months ago' 
      }
    ];
    ...
    // If the API successfully returns text reviews, format them
